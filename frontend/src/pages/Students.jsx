    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative flex-shrink-0">
          ...
          />
          {search && (
            <button onClick={() => setSearch('')} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-700 hover:text-slate-400">
              <X size={12} />
            </button>
          )}
        </div>
      </div>
      <p className="text-[11px] text-slate-700 font-mono -mt-3">{filtered.length} of {students.length} students</p>
            ...
            <table className="w-full text-left min-w-[700px]">
              <thead>
                <tr className="border-b border-[#1D1D2D]">
                  <th className="py-3.5 px-5 text-[10px] font-semibold text-slate-700 tracking-[0.15em] uppercase">Student</th>
                  <th className="py-3.5 px-5 text-[10px] font-semibold text-slate-700 tracking-[0.15em] uppercase">Exam</th>
                  <th className="py-3.5 px-5 text-[10px] font-semibold text-slate-700 tracking-[0.15em] uppercase">Phone</th>
                  <th className="py-3.5 px-5 text-[10px] font-semibold text-slate-700 tracking-[0.15em] uppercase">Telegram</th>
                  <th className="py-3.5 px-5 text-[10px] font-semibold text-slate-700 tracking-[0.15em] uppercase text-right">Joined</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((s) => (
                  <tr key={s.telegram_id} className="border-b border-[#1D1D2D]/60 hover:bg-[#18182A] transition-colors last:border-0">
                    <td className="py-4 px-5">
                      <div className="flex items-center gap-3">
                        <div
                          className="w-9 h-9 rounded-lg flex items-center justify-center text-white text-xs font-bold flex-shrink-0"
                          style={{ background: `${({JEE:"#F97316",NEET:"#2DD4BF",SSC:"#38BDF8",UPSC:"#A3E635",CUET:"#FBBF24"}[s.exam_preference]||"#6366F1")}18`, border: `1px solid ${({JEE:"#F97316",NEET:"#2DD4BF",SSC:"#38BDF8",UPSC:"#A3E635",CUET:"#FBBF24"}[s.exam_preference]||"#6366F1")}30` }}
                        >
                          {getInitials(s.name)}
                        </div>
                        <div>
                    ...
                    <td className="py-4 px-5"><ExamBadge exam={s.exam_preference} /></td>
                    <td className="py-4 px-5">
                      {s.phone_number ? (
                        <div className="flex items-center gap-1.5">
                          <Phone size={12} className="text-[#22D3EE]" />
                          <span className="text-sm text-[#22D3EE] font-mono">{s.phone_number}</span>
                        </div>
                      ) : (
                    ...
                    <td className="py-4 px-5">
                      {s.username ? (
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-sm text-slate-500 font-mono">{s.username}</span>
                        </div>
                      ) : (
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan="5" className="py-16 text-center">
                      ...
                      <p className="text-sm text-slate-700">{search ? `No results for "${search}"` : `No students in ${activeFilter}.`}</p>
                    </td>
                  </tr>
                )}
              </tbody>
