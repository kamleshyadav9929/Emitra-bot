    ...
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center px-4">
      <div className="w-full max-w-md bg-white shadow-xl rounded-[24px] overflow-hidden">
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 bg-[#F8FAFC]">
          <div>
            <p className="text-[10px] text-[#164FA8] font-bold tracking-widest uppercase">{service ? "Edit Service" : "Add Service"}</p>
            <h2 className="text-[16px] font-bold text-gray-900 mt-1">{service?.name || "New Service"}</h2>
          </div>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-3 bg-[#F0F4FA] p-4 rounded-[16px]">
              <label className="text-[13px] font-bold text-gray-900 flex-1">Active in Telegram Bot</label>
            ...
            <div className="flex items-center gap-3 bg-[#F0F4FA] p-4 rounded-[16px]">
              <label className="text-[13px] font-bold text-gray-900 flex-1">Active on Web Portal</label>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[11px] font-bold tracking-wider text-[#164FA8] mt-1 uppercase">✓ Realtime Sync Enabled</p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {Object.entries(grouped).map(([catLabel, items]) => (
            <div key={catLabel} className="border border-gray-100 bg-white rounded-[24px] overflow-hidden shadow-sm">
              <div className="px-6 py-4 bg-[#F8FAFC] border-b border-gray-50">
                <p className="text-[11px] font-bold tracking-widest text-[#164FA8] uppercase">{catLabel}</p>
              </div>
              {items.map((s, i) => (
                <div key={s.id} className={`flex flex-col sm:flex-row sm:items-center gap-4 px-6 py-5 hover:bg-[#F0F4FA]/50 transition-colors ${i < items.length - 1 ? 'border-b border-gray-50' : ''}`}>
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 flex-wrap">
                          <p className={`text-[14px] font-bold ${s.enabled || s.show_in_web ? "text-gray-900" : "text-gray-400"}`}>{s.name}</p>
                          {s.price && <span className="text-[11px] font-bold bg-[#EBF1FA] text-[#164FA8] px-2 py-0.5 rounded-full">{s.price}</span>}
                          <div className="flex items-center gap-1.5 ml-1">
                             <span className={`text-[9px] font-bold tracking-widest uppercase px-2 py-0.5 rounded-full ${s.enabled ? "bg-emerald-50 text-[#10B981]" : "bg-gray-100 text-gray-400"}`}>Bot: {s.enabled ? "ON" : "OFF"}</span>
                             <span className={`text-[9px] font-bold tracking-widest uppercase px-2 py-0.5 rounded-full ${s.show_in_web ? "bg-emerald-50 text-[#10B981]" : "bg-gray-100 text-gray-400"}`}>Web: {s.show_in_web ? "ON" : "OFF"}</span>
                          </div>
                        </div>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0 self-end sm:self-auto mt-2 sm:mt-0">
                    <button onClick={() => setModal(s)} className="w-9 h-9 border border-gray-200 bg-white shadow-sm flex items-center justify-center text-gray-500 hover:border-[#164FA8] hover:text-[#164FA8] transition-all rounded-[12px]" title="Edit">
                      <Pencil size={14} />
                    </button>
                    <button onClick={() => handleDelete(s.id)} className="w-9 h-9 border border-gray-200 bg-white shadow-sm flex items-center justify-center text-gray-500 hover:border-red-500 hover:text-red-500 hover:bg-red-50 transition-all rounded-[12px]" title="Delete">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
